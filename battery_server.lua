...
local batteryNames = {
  ["d76c"] = "Battery1" -- Here you can provide names of your batteries by their addresses. Address can be full guid or first few symbols.
}
...
function Substation.new(proxy, name)
  ...
  local config = {
    ...
    TOTAL_INPUT = {line = 10, pattern = "^.+§.(.+)§.+$"},
    TOTAL_OUTPUT = {line = 11, pattern = "^.+§.(.+)§.+$"},
    TOTAL_COSTS = {line = 12, pattern = "^.+§.(.+)§.+$"}
  }
...
function Main()
  ...
  repeat
    ...
    for _, battery in ipairs(values.batteries) do
      ...
      outStr = battery.name .. "|" .. battery.stored .. "|" .. battery.capacity .. "|" .. battery.input .. "|" .. battery.output  .. "|" .. battery.percent .. "|" .. battery.charge .. "|" .. battery.left .. "@"
    end
